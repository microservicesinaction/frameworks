...
object Colour {
  implicit val colourFormat: Format[Colour] = Json.format[Colour]
}

case class AttributeTag(value: String)

object AttributeTag {
  implicit val attributeTagFormat: Format[AttributeTag] = Json.format[AttributeTag]
}
...
case class Item(id: Option[Int], price: Price, colour: Colour, tags: AttributeTag, size: Size, state: State, sku: Sku)
...
case class ItemRecord(id: Int, priceValue: Int, priceCurrency: String, colour: String, tags: String, sizeValue: Int, sizeRegion: String, state: String, sku: String) {
  def toItem: Item = Item(id = Some(id), price = Price(priceValue, priceCurrency), colour = Colour(colour), tags = AttributeTag(tags), size = Size(sizeValue, sizeRegion), state = State(state), sku = Sku(sku))
}
